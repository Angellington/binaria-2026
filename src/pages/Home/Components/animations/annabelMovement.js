...
const MAX_DEPTH_SCALE = 1.18;
const MASK_VISIBLE_UNTIL = 1;
const MASK_FADE_END = 1;
...
const getMaskOpacity = (localX, radiusX) => {
  ...
  return (
    1 - (xPercent - MASK_VISIBLE_UNTIL) / (MASK_FADE_END - MASK_VISIBLE_UNTIL)
  );
};
...
export const startAnnabelMovement = (element, orbitalClassName) => {
  ...
  const moveNagao = (timestamp) => {
    ...
    const orbitX =
      orbitCenterX + matrix.a * localX + matrix.c * localY + matrix.e;
    const orbitY =
      orbitCenterY + matrix.b * localX + matrix.d * localY + matrix.f;
